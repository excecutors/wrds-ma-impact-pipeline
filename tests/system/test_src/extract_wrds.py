# This is not used for the system test. The fixtures will be used instead. However, it is worth keeping here in case an alternative testing method that requires this file is needed.
import os
import gc
import sys
import wrds
...
def get_wrds_connection():
    ...
    try:
        ...
        logging.error(
            "Please ensure that your WRDS credentials have been correctly set up either through .pgpass or the login prompt."
        )
        raise
...
def extract_and_load(db_wrds, db_postgres_engine):
    ...
    try:
        logging.info(
            "Extracting filtered 'ot_glb_company' (North America Public Only)..."
        )
        ...
        list_north_america_public_companyid = (
            df_company["companyid"].dropna().unique().tolist()
        )
        list_north_america_public_ticker = (
            df_company["ticker"].dropna().unique().tolist()
        )
