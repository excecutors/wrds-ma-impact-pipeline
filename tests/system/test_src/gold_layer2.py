...
def transform_gold(output_dir="data"):
    """
    Gold Layer Logic:
    ...
    df_calc = df_silver.with_columns(
        [
            # --- Pre-Deal ---
            (pl.col("stock_price_pre") * pl.col("shares_outstanding_pre")).alias(
                "market_cap_pre"
            ),
            (
                (
                    pl.col("long_term_debt_pre").fill_null(0)
                    + pl.col("current_debt_pre").fill_null(0)
                )
            ).alias("total_debt_pre"),
            # --- Post-Deal ---
            (pl.col("stock_price_post") * pl.col("shares_outstanding_post")).alias(
                "market_cap_post"
            ),
            (
                (
                    pl.col("long_term_debt_post").fill_null(0)
                    + pl.col("current_debt_post").fill_null(0)
                )
            ).alias("total_debt_post"),
        ]
    )
    ...
    df_calc = df_calc.with_columns(
        [
            (
                pl.col("market_cap_pre")
                + pl.col("total_debt_pre")
                - pl.col("cash_pre").fill_null(0)
            ).alias("ev_pre"),
            (
                pl.col("market_cap_post")
                + pl.col("total_debt_post")
                - pl.col("cash_post").fill_null(0)
            ).alias("ev_post"),
        ]
    )
    ...
    df_gold = df_calc.with_columns(
        [
            ...
            ((pl.col("ev_post") - pl.col("ev_pre")) / pl.col("ev_pre")).alias(
                "delta_ev_pct"
            ),
            # 2. Market Cap Growth % (Robustness Check)
            (
                (pl.col("market_cap_post") - pl.col("market_cap_pre"))
                / pl.col("market_cap_pre")
            ).alias("delta_mkt_cap_pct"),
            # 3. EBITDA Growth % (Profitability Change)
            (
                (pl.col("ebitda_post") - pl.col("ebitda_pre")) / pl.col("ebitda_pre")
            ).alias("delta_ebitda_pct"),
    ...
    logging.info(
        f"Data Cleaning: {initial_count} -> {final_count} rows (Dropped {initial_count - final_count})"
    )
    ...
    df_final.to_pandas().to_sql(
        "final_data", engine, schema="gold", if_exists="replace", index=False
    )
