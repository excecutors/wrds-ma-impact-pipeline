...
def transform_silver():
    ...
    logging.info(
        f"Loaded: Deal({df_deal.height}), Rel({df_rel.height}), Comp({df_comp.height}), Industry({df_industry.height}), Fund({df_fund.height})"
    )
    ...
    df_deal = df_deal.with_columns(
        pl.col("announceddate").cast(pl.Date), pl.col("dealdate").cast(pl.Date)
    )
    ...
    df_deal = df_deal.rename(
        {"companyid": "target_company_id", "companyname": "target_company_name"}
    )
    df_comp = df_comp.rename(
        {
            "companyid": "acquirer_company_id",
            "companyname": "acquirer_name",
    ...
    df_merged = df_merged.join(df_comp, on="acquirer_company_id", how="inner")

    # 3.3 Acquirer -> Industry
    df_merged = df_merged.join(
        df_industry, left_on="acquirer_ticker", right_on="ticker", how="left"
    )
    ...
    logging.info(
        f"Dropped {initial_count - final_count} rows due to missing data. Final count: {final_count}"
    )
    ...
    logging.info(
        f"Successfully wrote {len(df_final)} rows to silver.deal_financials_linked"
    )
